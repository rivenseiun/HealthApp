...
import com.whu.healthapp.set.BootSetting;
import com.whu.healthapp.utils.DateUtils;
import com.whu.healthapp.view.Sphygmomanometer;
...
public class BloodActivity extends BaseActivity implements ResponseListener {

    private Sphygmomanometer surfaceBp;
    ...
    private TextView tvXiudai, tvShousuoya, tvShuzhangya, tvPingjunya;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_test_bloodp);
        //保持屏幕常量
...
//在释放之前，屏幕一直亮着（有可能会变暗,但是还可以看到屏幕内容,换成PowerManager.SCREEN_BRIGHT_WAKE_LOCK不会变暗）
        wl.acquire();
        surfaceBp = (Sphygmomanometer) findViewById(R.id.surface_bp);
        tvXiudai = (TextView) findViewById(R.id.tv_xiudaiya);
        tvShousuoya = (TextView) findViewById(R.id.tv_shousuoya);
        tvShuzhangya = (TextView) findViewById(R.id.tv_shuzhangya);
        tvPingjunya = (TextView) findViewById(R.id.tv_pingjunya);
...
//	    	    imgSensorGuide.setVisibility(View.GONE);
        ...
        //自动测量血压
        autoTime = BootSetting.getAutoApTime(this);
        if (autoTime != 0) {
            autoTestBp();
        }
    }
    ...
    public void autoTestBp() {
        handler.sendEmptyMessageDelayed(0, autoTime);
    }
    ...
    private XueYaHandler handler = new XueYaHandler(this);

    private static class XueYaHandler extends Handler {
        WeakReference<Activity> activity;

        XueYaHandler(Activity activity) {
        ...
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            if ((BloodActivity) activity.get() != null && ((BloodActivity) activity.get()).isTesting) {
                ((BloodActivity) activity.get()).sendCommandToBluetooth(1); //发送测量血压的命令
                ((BloodActivity) activity.get()).autoTestBp();
            }
        }
    }
    ...
    int bpnibps, bphigh, bplow, bppulse, bpcuff, bpavg; //NIBP血压状态、收缩压、舒张压、血压脉率、袖带压力、平均压力
    ...
    Runnable showDataRunnable = new Runnable() {
        public void run() {
            if (isTesting) {
//                if (bpavg == 0)
//                    tvXiudai.setText("测量中");
//                else
                tvXiudai.setText(bpcuff * 2 + "");
//                if (bphigh == 0)
//                    tvShousuoya.setText("测量中");
//                else
                tvShousuoya.setText(bphigh + "");
//                if (bplow == 0)
//                    tvShuzhangya.setText("测量中");
//                else
                tvShuzhangya.setText(bplow + "");
//                if (bpavg == 0)
//                    tvPingjunya.setText("测量中");
//                else
                tvPingjunya.setText(bpavg + "");
                surfaceBp.setTargetTemperature(bpcuff * 2);
            }
        }
    };
    ...
    public void startMeasurement() {
        ...
        sendCommandToBluetooth(10); //血压清零
        ...
        sendCommandToBluetooth(1); //发送测量血压的命令
    }
                ...
                new AlertDialog.Builder(this).setTitle("提示").setMessage("是否直接退出？").setNegativeButton("取消", null).setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (bluetoothThread != null) {
                            sendCommandToBluetooth(2); //紧急终止血压测量
                            stopBluetoothThread();
                        }
...
//	    	    imgSensorGuide.setVisibility(View.GONE);
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            //开启蓝牙测试
                            bluetoothThread = new BluetoothConnectThread(BluetoothDevice.getDeviceMac(BloodActivity.this)); //创建bluetoothThrad
                            if (!bluetoothThread.checkBtAdapterExists()) {
                                Toast.makeText(getBaseContext(), getResources().getString(R.string.nobluetooth), Toast.LENGTH_SHORT).show();
                                finish();
                            }
            ...
            case R.id.tv_stop:
                ...
                if (bluetoothThread != null) {
                    ...
                    data.setKind(TestDataBean.TEST_XUEYA);
                    data.setShuzhangya(bplow + "");
                    data.setShousuoya(bphigh + "");
                    data.setAveya(bpavg + "");
                    data.setXiudaiya(bpcuff + "");
                    data.setUserId(User.getInstance(BloodActivity.this).getPhone());
                    data.setTime(DateUtils.getCurrentDate());
                    XueyaDB db = new XueyaDB(this);
                    db.addData(data);
                    ...
                    String url = "http://47.92.55.20:80/Healthy/data.mvc?action=1";
                    RequestParams params = new RequestParams(url);
                    String phone = User.getInstance(BloodActivity.this).getPhone();
                    params.addBodyParameter("phone", phone);
                    params.addBodyParameter("diastolic", String.valueOf(bplow));
                    params.addBodyParameter("average", String.valueOf(bpavg));
                    params.addBodyParameter("systolic", String.valueOf(bphigh));
                    x.http().post(params, new org.xutils.common.Callback.CommonCallback<String>() {
                        ...
                        public void onError(Throwable ex, boolean isOnCallback) {
                            Toast.makeText(BloodActivity.this, "上传失败", Toast.LENGTH_LONG).show();
                        ...
                        public void onSuccess(String result) {
                            if(result.equals("error")){
                                Toast.makeText(BloodActivity.this, "错误：用户不存在", Toast.LENGTH_LONG).show();
                            }
                            else {
                                Toast.makeText(BloodActivity.this, "上传成功", Toast.LENGTH_LONG).show();
                                ...
                                Intent intent=new Intent(BloodActivity.this, AssesBlood.class);
                                intent.putExtra("shuzhangya",tvShuzhangya.getText());
                                intent.putExtra("shousuoya",tvShousuoya.getText());
                                intent.putExtra("aveya",tvPingjunya.getText());
